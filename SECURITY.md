...
- **`setup.sh` writes only inside `<project>/.fagents-tty/` plus `~/.fagents-tty/registry/<project>/.path`.** It never modifies `.claude/settings.json`, `.codex/hooks.json`, or top-level launcher scripts, and never installs the agent skill globally. Test 35 hashes the JSON files before and after `setup.sh` to prove byte-for-byte preservation.
...
- [ ] No global state written outside `~/.fagents-tty/registry/` and `<project>/.fagents-tty/` (no `~/.claude/`, no `~/.codex/`, no `.claude/settings.json`).
